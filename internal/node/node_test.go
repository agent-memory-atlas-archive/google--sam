...
func TestShouldDialDiscovered(t *testing.T) {
	...
	node := &SamNode{Host: host, revokedPeers: revokedCache}

	_, revokedPub, err := crypto.GenerateEd25519Key(nil)
	if err != nil {
		t.Fatal(err)
	}
	revoked, err := peer.IDFromPublicKey(revokedPub)
	if err != nil {
		t.Fatal(err)
	}
	node.revokedPeers.Add(revoked.String(), time.Now().UnixMilli())

	_, healthyPub, err := crypto.GenerateEd25519Key(nil)
	if err != nil {
		t.Fatal(err)
	}
	healthy, err := peer.IDFromPublicKey(healthyPub)
	if err != nil {
		t.Fatal(err)
	}

	if node.shouldDialDiscovered(revoked) {
		t.Error("revoked peer should not be dialled from DHT discovery")
	}
	if node.shouldDialDiscovered(host.ID()) {
		t.Error("self should not be dialled from DHT discovery")
	}
	if !node.shouldDialDiscovered(healthy) {
		t.Error("healthy peer should be dialled from DHT discovery")
	}
}
